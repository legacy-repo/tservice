(ns tservice.task
  "Background task scheduling via Quartzite. Individual tasks are defined in `tservice.task.*`.
...
(defn- find-and-load-task-namespaces!
  "Search Classpath for namespaces that start with `tservice.tasks.`, then `require` them so initialization can happen."
  []
  (doseq [ns-symb @u/tservice-namespace-symbols
          :when   (.startsWith (name ns-symb) "tservice.task.")]
    (try
